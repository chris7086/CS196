...
# 
#     * Output: 
#         * `2` The cycles here are the indices 0,1,3,4 and 2,5,6,7 
# 
# ## Parameters
# -----------
# * `list` : `arr`
...
def count_cycles(arr):
    ...
    for x in range (0,len(arr)):
        ...
        loop = False
        a = arr[x]
        while (i <= len(arr) and loop == False):
            if (a==x):
                loop = True
                count = count + 1/(i)
            i = i + 1
...
def string_permutation_is_palindrome(input):
    dict = {}
    for char in input:
        if char not in dict and char != ' ':
            dict[char] = 1
        elif char in dict and char != ' ':
